...
public class MachineReader extends YamlReader<AbstractEmptyMachine<?>> {
    ...
    public AbstractEmptyMachine<?> readEach(String s) {
        ...
        Pair<ExceptionHandler.HandleResult, RecipeType> rt = ExceptionHandler.getRecipeType(
                "Found an error while loading machine " + s + " in addon " + addon.getAddonId() + ": Invalid recipe type '" + recipeType + "'!", recipeType);
        ...
        if (section.contains("script")) {
            ...
            if (!file.exists()) {
                ExceptionHandler.handleWarning("There was an error while loading machine " + s + " in addon " + addon.getAddonId() + ": " + "Could not find script file " + file.getName());
            } else {
        ...
        if (section.contains("energy")) {
            ...
            if (energySettings == null) {
                ExceptionHandler.handleWarning("There was an error while loading machine " + s + " in addon " + addon.getAddonId() + ": " + "Could not find energy settings! The machine will be treated as a no-energy machine.");
            ...
            if (capacity < 0) {
                ExceptionHandler.handleError("There was an error while loading machine " + s + " in addon " + addon.getAddonId() + ": " + "Capacity cannot be negative! The machine will be treated as a no-energy machine.");
                machine = new CustomNoEnergyMachine(
                        group.getSecondValue(),
                        slimefunItemStack,
                        rt.getSecondValue(),
            ...
            Pair<ExceptionHandler.HandleResult, EnergyNetComponentType> enc = ExceptionHandler.handleEnumValueOf(
                    "Found an error while loading machine " + s + " in addon " + addon.getAddonId() + ": Invalid energy network component type '" + encType + "'! The machine will be treated as a no-energy machine.",
    ...
    public List<SlimefunItemStack> preloadItems(String s) {
        ...
        if (stack == null) {
            ExceptionHandler.handleError("Found an error while loading machine " + s + " in addon " + addon.getAddonId() + ": " + "The item is null or has an invalid format");
            return null;
        }
