...
public class SimpleMachineReader extends YamlReader<SlimefunItem> {
    ...
    public SlimefunItem readEach(String s) {
        ...
        Pair<ExceptionHandler.HandleResult, SimpleMachineType> machineTypePair = ExceptionHandler.handleEnumValueOf(
                "Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Invalid simple machine type: "  + machineTypeStr, SimpleMachineType.class, machineTypeStr);
        ...
        if (settings == null) {
            ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Machine has no settings");
            return null;
        }
        ...
        if (machineType.isEnergy()) {
            ...
            if (capacity < 1) {
                ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Energy capacity is less than 1");
                return null;
            }

            consumption = settings.getInt("consumption");
            if (consumption < 1) {
                ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Energy consumption is less than 1");
                return null;
            }
            ...
            if (!isAccelerator(machineType)) {
                ...
                if (speed < 1) {
                    ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Speed is less than 1");
                    return null;
                }
            } else {
                radius = settings.getInt("radius", 1);
                if (radius < 1) {
                    ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Radius is less than 1");
                    return null;
                }
            ...
            if (machineType == SimpleMachineType.AUTO_ANVIL) {
                ...
                if (repairFactor < 1) {
                    ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": Repair factor is less than 1");
                    return null;
                }
            }
        }
    ...
    public List<SlimefunItemStack> preloadItems(String s) {
        ...
        if (stack == null) {
            ExceptionHandler.handleError("Found an error while loading simple machine " + s + " in addon " + addon.getAddonId() + ": " + "The item is null or has an invalid format");
            return null;
        }
